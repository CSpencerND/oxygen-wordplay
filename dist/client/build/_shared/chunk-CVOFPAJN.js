...
var Ce = "private";
var Te = "no-store";
var ee = { maxAge: "max-age", staleWhileRevalidate: "stale-while-revalidate", sMaxAge: "s-maxage", staleIfError: "stale-if-error" };
function E(e) {
  let t = [];
  return Object.keys(e).forEach((r) => {
    r === "mode" ? t.push(e[r]) : ee[r] && t.push(`${ee[r]}=${e[r]}`);
  }), t.join(", ");
}
function F() {
  return { mode: Te };
...
function te(e) {
  if (e?.mode && e?.mode !== v && e?.mode !== Ce)
    throw Error("'mode' must be either 'public' or 'private'");
}
function w(e) {
  return te(e), { mode: v, maxAge: 1, staleWhileRevalidate: 9, ...e };
}
function N(e) {
  return te(e), { mode: v, maxAge: 3600, staleWhileRevalidate: 82800, ...e };
}
var M = "Error in SEO input: ";
var H = { title: { validate: (e) => {
...
function ye(e) {
  ...
  for (let r of Object.keys(e)) {
    ...
    let p = o.map((i) => {
      ...
      switch (r) {
        case "title": {
          s = j(H.title, i);
          let f = Ne(e?.titleTemplate, s);
          a.push(h("title", { title: f }), h("meta", { property: "og:title", content: f }), h("meta", { name: "twitter:title", content: f }));
          break;
        }
        case "description":
          s = j(H.description, i), a.push(h("meta", { name: "description", content: s }), h("meta", { property: "og:description", content: s }), h("meta", { name: "twitter:description", content: s }));
          break;
        case "url":
          s = j(H.url, i), a.push(h("meta", { property: "og:url", content: s }), h("link", { rel: "canonical", href: s }));
          break;
        case "handle":
          s = j(H.handle, i), a.push(h("meta", { name: "twitter:site", content: s }), h("meta", { name: "twitter:creator", content: s }));
          break;
        case "media": {
          let f = z(i);
          for (let g2 of f)
            if (typeof g2 == "string" && a.push(h("meta", { name: "og:image", content: g2 })), g2 && typeof g2 == "object") {
              let A = g2.type || "image", R = g2 ? { url: g2?.url, secure_url: g2?.url, type: $e(g2.url), width: g2?.width, height: g2?.height, alt: g2?.altText } : {};
              for (let b of Object.keys(R))
                R[b] && (s = R[b], a.push(h("meta", { property: `og:${A}:${b}`, content: s }, R.url)));
            }
          break;
        }
        case "jsonLd": {
          let f = h("script", { type: "application/ld+json", children: JSON.stringify(i) }, `json-ld-${i?.["@type"] || i?.name || ""}`);
          a.push(f);
          break;
        }
        case "alternates": {
          ...
          for (let g2 of f) {
            if (!g2)
              continue;
            let { language: A, url: R, default: b } = g2, _ = A ? `${A}${b ? "-default" : ""}` : void 0;
            a.push(h("link", { rel: "alternate", hrefLang: _, href: R }));
          }
          break;
        }
        case "robots":
          let { maxImagePreview: d, maxSnippet: m, maxVideoPreview: x, noArchive: l2, noFollow: C, noImageIndex: u2, noIndex: T, noSnippet: I, noTranslate: y, unavailableAfter: S } = i, U = [l2 && "noarchive", u2 && "noimageindex", I && "nosnippet", y && "notranslate", d && `max-image-preview:${d}`, m && `max-snippet:${m}`, x && `max-video-preview:${x}`, S && `unavailable_after:${S}`], P = (T ? "noindex" : "index") + "," + (C ? "nofollow" : "follow");
          for (let f of U)
            f && (P += `,${f}`);
          a.push(h("meta", { name: "robots", content: P }));
          break;
      }
      return a;
    }).flat().filter((i) => !!i);
  ...
  return t.sort((r, n) => r.key.localeCompare(n.key));
}
function Ne(e, t) {
...
function h(e, t, r) {
  ...
  return e === "title" ? (n.children = t.title, n.key = J(n), n) : e === "script" ? (n.children = t.children, n.key = J(n, r), delete t.children, n.props = t, n) : (n.props = t, Object.keys(n.props).forEach((o) => !n.props[o] && delete n.props[o]), n.key = J(n, r), n);
}
function J(e, t) {
  let { tag: r, props: n } = e;
  if (r === "title")
  ...
  if (r === "meta") {
    ...
    return [r, ...[t, o], n.property || n.name].filter((c) => c).join("-");
  }
  return r === "link" ? [r, n.rel, n.hrefLang || n.media].filter((s) => s).join("-").replace(/\s+/g, "-") : r === "script" ? `${r}-${t}` : `${r}-${n.type}`;
}
function z(e) {
...
var Ke = (0, import_react7.lazy)(() => import("/build/_shared/log-seo-tags-N7J66FN6-WB3VXEMS.js"));
function Ye({ debug: e }) {
  let t = useMatches(), r = useLocation(), n = (0, import_react7.useMemo)(() => t.flatMap((c) => {
    let { handle: p, ...i } = c, a = { ...i, ...r }, d = p?.seo, m = i?.data?.seo;
    return !d && !m ? [] : d ? q(p.seo, a) : [m];
  }).reduce((c, p) => {
    Object.keys(p).forEach((a) => !p[a] && delete p[a]);
    let { jsonLd: i } = p;
    return i ? c?.jsonLd ? Array.isArray(i) ? { ...c, ...p, jsonLd: [...c.jsonLd, ...i] } : { ...c, ...p, jsonLd: [...c.jsonLd, i] } : { ...c, ...p, jsonLd: [i] } : { ...c, ...p };
  }, {}), [t, r]), { html: o, loggerMarkup: s } = (0, import_react7.useMemo)(() => {
    let c = ye(n), p = c.map((a) => a.tag === "script" ? (0, import_react7.createElement)(a.tag, { ...a.props, key: a.key, dangerouslySetInnerHTML: { __html: a.children } }) : (0, import_react7.createElement)(a.tag, { ...a.props, key: a.key }, a.children)), i = (0, import_react7.createElement)(import_react7.Suspense, { fallback: null }, (0, import_react7.createElement)(Ke, { headTags: c }));
    return { html: p, loggerMarkup: i };
  }, [n]);
  return (0, import_react7.createElement)(import_react7.Fragment, null, o, e && s);
}
function q(e, ...t) {
  if (e instanceof Function)
    return q(e(...t), ...t);
  let r = {};
  return Array.isArray(e) ? (r = e.reduce((n, o) => [...n, q(o)], []), r) : e instanceof Object ? (Object.entries(e).forEach(([o, s]) => {
    r[o] = q(s, ...t);
  }), r) : e;
}
...
//# sourceMappingURL=/build/_shared/chunk-CVOFPAJN.js.map
